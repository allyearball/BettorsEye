...
def main():
    ...
    # Merge in VSiN's DraftKings Handle %/Bet % splits (full-game Moneyline/Spread/Total
    # only — see scrape_vsin_splits.py). Best-effort: a scrape hiccup (VSiN layout change,
    # network blip, no games today) shouldn't fail the whole build — the Odds page already
    # renders "VSiN: —" gracefully when this key is missing or a team isn't present in it.
    try:
